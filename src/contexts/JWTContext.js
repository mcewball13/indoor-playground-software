...
const handlers = {
  ...
  CUSTOMER_EXISTS_LOADING: (state, action) => {
    const { emailExistsLoading } = action.payload;
    return {
      ...state,
      emailExistsLoading,
    };
...
function AuthProvider({ children }) {
  ...
  const customerExists = async (email) => {
    dispatch({
      type: 'CUSTOMER_EXISTS_LOADING',
      payload: {
        emailExistsLoading: true,
      },
    });
    // change to axios.post when we have a completed backend
    // // =========================================================================
